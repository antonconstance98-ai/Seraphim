...
- [x] **EXEC-01**: `codex-exec.js` forked from v2.0, adapted to unified interface (`execute`, `stream`, `available`), fallback chain removed (dispatch owns it)
- [x] **EXEC-02**: `minimax-exec.js` forked from v2.0, adapted to unified interface, `runWithFallback` removed, temperature 0.01 preserved
...
- [x] **EXEC-04**: `qwen-exec.js` runs Qwen locally via ollama HTTP at localhost:11434 with 180s timeout, warm-up probe, `num_ctx: 32768`, and JSON structured output for Forge mode
- [x] **EXEC-05**: `perplexity-exec.js` integrates Perplexity Sonar via OpenAI SDK baseURL swap (`https://api.perplexity.ai`), extracts citations from `response.citations`
...
- [x] **EXEC-07**: `qwen-exec.js` `available()` sends a minimal inference probe (not just `/api/tags` check) and returns false gracefully when GPU/ollama is absent
...
- [x] **QUAL-03**: Judge->Envision feedback loop: when all approaches get FATAL_FLAW, Envision re-runs with Judge's findings (max 2 loops, persisted to disk)
...
- [x] **DASH-04**: Localhost web dashboard at `127.0.0.1:PORT` serves a self-contained HTML interface (Node.js HTTP server, no frameworks)
- [x] **DASH-05**: Dashboard shows multi-project overview — each project's name, profile, current phase, progress bar, total cost, last activity
- [x] **DASH-06**: Per-project drill-down shows phase roadmap, completed vs remaining tasks, model assignments, and pipeline run history
- [x] **DASH-07**: Workflow metrics panel shows cross-project model performance, cost trends, and savings vs Opus-only baseline (extends existing Chart.js dashboard patterns)
